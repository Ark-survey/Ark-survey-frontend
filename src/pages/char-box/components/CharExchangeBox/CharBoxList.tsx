...
export default function CharacterList() {
  ...
  const list = useMemo(() => {
    return mapToArray<CharacterType>(charData)
      .filter((it) => {
        return (
          !it.isNotObtainable && Object.keys(charInBox).filter((i) => i === it.key).length > 0 && filterCharFunc(it)
        );
      })
      .map((character) => (
        <CharContainer
  ...
  }, [charData, charInBox, dispatch, editingCharKey, filterCharFunc, mini, nameDisplay]);
